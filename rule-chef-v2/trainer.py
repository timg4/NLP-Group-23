...
FEEDBACKS = {
    "ORG": (
        "ORG must include a legal form/keyword (GmbH, AG, SE, KG, Bank, Sparkasse, "
        "Ministerium, Amt, Behoerde). Never use generic capitalization patterns or "
        "all-caps-only rules."
    ),
    "MON": (
        "MON must include at least one digit AND a currency/amount marker or %/Prozent. "
    ),
    "LEG": (
        "LEG must include §/Art./Abs./Nr./Satz/lit. or law abbreviations (BGB, HGB, DSGVO, AktG). "
    ),
}
